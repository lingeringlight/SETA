...
from .gfnet_patch_mix import GFNetPyramid
...
import random
from timm.data import Mixup as Mixup_Function
...
class GFNet(Algorithm):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(GFNet, self).__init__(input_shape, num_classes, num_domains,
                                  hparams, )
        self.network = GFNetPyramid(
            img_size=224,
            patch_size=4,
            ...
            noise_mode=self.hparams["noise_mode"],
            global_filter=1,
            uncertainty_model=self.hparams["uncertainty_model"],
            uncertainty_factor=self.hparams["uncertainty_factor"],
            MixStyle_flag=self.hparams["MixStyle_flag"],
        )

        if hparams["optimizer"] == 1:
            linear_scaled_lr = self.hparams["lr"] * self.hparams["batch_size"] * 1 / 512.0
            self.optimizer = create_optimizer_v2(self.network, 'adamw',
                                            linear_scaled_lr,
        ...
        model_path = "/GFNet-master/pretrained_model/"
        self.pretrained(model_path=model_path)

        self.mixup_alpha = self.hparams["mixup_alpha"]
        self.cutmix_alpha = self.hparams["cutmix_alpha"]
        mixup_active = self.mixup_alpha > 0 or self.cutmix_alpha > 0.
        self.mixup_fn = None
        if mixup_active:
            print('standard mix up')
            self.mixup_fn = Mixup_Function(
                mixup_alpha=self.mixup_alpha, cutmix_alpha=self.cutmix_alpha, num_classes=num_classes, prob=0.5)
            # self.criterion = SoftTargetCrossEntropy()


    def pretrained(self, model_path):
        # load_pretrained_model
        pretrained_model = model_path + "gfnet-h-ti.pth"
        ...
        for name in checkpoint_model.keys():
            if 'complex_weight' in name:
                ...
                new_weight = torch.nn.functional.interpolate(
                    origin_weight, size=(upsample_h, upsample_w), mode='bicubic', align_corners=True).\
                    permute(0, 2, 3, 1).reshape(upsample_h, upsample_w, num_heads, 2)
                checkpoint_model[name] = new_weight

        self.network.load_state_dict(checkpoint_model, strict=False)

    def update(self, minibatches, unlabeled=None):
        all_x = torch.cat([x for x, y in minibatches])
        all_y = torch.cat([y for x, y in minibatches])

        # if self.mixup_fn != None:
        #     all_x, all_y = self.mixup_fn(all_x, all_y)
        #     loss = self.criterion(self.predict(all_x), all_y.long())
        # else:
        #     loss = F.cross_entropy(self.predict(all_x), all_y.long())
...
class GFNet_SETAug(Algorithm):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(GFNet_SETAug, self).__init__(input_shape, num_classes, num_domains,
                                  hparams, )
        self.hparams = hparams
        self.PatchMix_layer = self.hparams["PatchMix_layer"]
        self.network = GFNetPyramid(
            img_size=224,
            patch_size=4,
            num_classes=num_classes,
            ...
            patchmix_prob=self.hparams["patchmix_prob"],
            fourier_mix_or_cutmix_prob=self.hparams["fourier_mix_or_cutmix_prob"],
            fourier_patch_mixup_alpha=self.hparams["fourier_patch_mixup_alpha"],
            fourier_patch_mixup_beta=self.hparams["fourier_patch_mixup_beta"],
            patch_cutmix_beta_flag=self.hparams["patch_cutmix_beta_flag"],

            phase_mask_size=self.hparams["phase_mask_size"],
            phase_batch_mean_flag=self.hparams["phase_batch_mean_flag"],
            phase_rfft_flag=self.hparams["phase_rfft_flag"],
            fourier_patch_phase_rate=self.hparams["fourier_patch_phase_rate"],

            fourier_ablation_flag=self.hparams["fourier_ablation_flag"],
            fourier_patch_protect_flag=self.hparams["fourier_patch_protect_flag"],
            stylize_mask_size=self.hparams["stylize_mask_size"],
            stylize_factor=self.hparams["stylize_factor"],
            DSU_flag=self.hparams["DSU_flag"],
        )
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        layer_index = np.random.randint(len(self.PatchMix_layer), size=1)[0]
        layer_select = [self.PatchMix_layer[layer_index]]

        if self.mixup_fn is not None:
            all_x, all_y = self.mixup_fn(all_x, all_y)

        loss = F.cross_entropy(self.predict(all_x, PatchMix_layer=layer_select), all_y)
    ...
    def predict(self, x, PatchMix_layer=None):
        return self.network(x, PatchMix_layer=PatchMix_layer).suip
