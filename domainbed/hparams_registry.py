...
def _hparams(algorithm, dataset, random_seed):
    ...
    elif algorithm == "GFNet_SETAug":
        _hparam('optimizer', 1, lambda r: 1)

        _hparam('patchmix_prob', 1.0, lambda r: 1.0)
        _hparam('fourier_patch_mixup_flag', 1, lambda r: 1)
        _hparam('fourier_mix_or_cutmix_prob', 0.8, lambda r: 0.8)
        _hparam('fourier_patch_mixup_alpha', 4.0, lambda r: 4.0)
        _hparam('fourier_patch_mixup_beta', 1.0, lambda r: 1.0)
        _hparam('patch_cutmix_beta_flag', 1, lambda r: 1)

        _hparam('phase_mask_flag', 0, lambda r: 0)
        _hparam('phase_mask_size', 0.6, lambda r: 0.6)
        _hparam('phase_batch_mean_flag', 0, lambda r: 0)
        _hparam('phase_rfft_flag', 0, lambda r: 0)

        _hparam('fourier_patch_phase_rate', 0.75, lambda r: 0.75)

        _hparam('fourier_ablation_flag', 0, lambda r: 0)
        _hparam('fourier_patch_protect_flag', 1, lambda r: 1)

        _hparam('stylize_mask_size', 0.6, lambda r: 0.6)
        _hparam('stylize_factor', 1.0, lambda r: 1.0)
        _hparam('DSU_flag', 0, lambda r: 0)

        _hparam('lr', 5e-4, lambda r: 5e-4)
        _hparam('PatchMix_layer', [0, 1, 2, 3], lambda r: [0, 1, 2, 3])

        _hparam('mixup_alpha', 0., lambda r: 0.)
        _hparam('cutmix_alpha', 0., lambda r: 0.)
