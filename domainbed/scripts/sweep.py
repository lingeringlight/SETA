...
def make_args_list(n_trials, dataset_names, algorithms, n_hparams_from, n_hparams, steps,
    data_dir, task, holdout_fraction, single_test_envs, hparams, device):
    args_list = []
...
if __name__ == "__main__":
    ...
    parser.add_argument('--datasets_name', type=str, default="PACS")
    parser.add_argument('--algorithms', nargs='+', type=str, default=algorithms.ALGORITHMS)
    parser.add_argument('--algorithms_name', default="DeiT_SETAug")
    ...
    s = args.hparams.replace("'", '"')
    hparams = json.loads(s)

    if args.algorithms_name == "DeiT_SETAug":
        args.output_dir += args.datasets_name + "/DeiT"
    elif args.algorithms_name == "SwinT_SETAug":
        args.output_dir += args.datasets_name + "/SwinT"
    elif args.algorithms_name == "ResNet_SETAug":
        args.output_dir += args.datasets_name + "/ResNet50-SETAug"
    else:
        args.output_dir += args.datasets_name + "/gfnet-h-ti"

    if hparams["MixStyle_flag"] == 1:
        args.output_dir += "_MixStyle"

    if hparams["mixup_alpha"] != 0:
        args.output_dir += "_image_mixup"
    if hparams["cutmix_alpha"] != 0:
        args.output_dir += "_image_cutmix"
